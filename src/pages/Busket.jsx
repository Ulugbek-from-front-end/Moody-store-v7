...
const Bag = () => {
  const { isEmpty, items, updateItemQuantity, TotalUniqueItems, removeItem, cartTotal } =
    useCart(); //a

  const CheckProducts = () => {
    if (isEmpty)
      return <Typography.Title level={2}>Bag is Empty</Typography.Title>;
        ...
        <Fragment key={product.id}>
          <Flex gap={18} className={`Shopping-bag__item`}>
            ...
            <div className="Shopping-bag__item-content">
              ...
              <Button
                icon={<IoMdClose />}onClick={()=>removeItem(product.id)}
            ...
            <Col span={10} className={`Shopping-bag__order`}>
              <form className={`Shopping-bag__form`}>
                <label>
                  <Title
                    level={"h3"}
                    className={`Shopping-bag__form-label__title`}
                  >
                    Add A DISCOUNT CODE
                  </Title>
                  <Flex>
                    ...
                    <Btn
                      ...
                      className={`Shopping-bag__form-button`}
                    >
                      ADD
                    </Btn>
                  </Flex>
                ...
                <label>
                  ...
                  <Btn secondary className={`Shopping-bag__form-button--white`}>
                    LOG IN
                  </Btn>
                </label>
                ...
                <ul className={`list-none Shopping-bag__form-list`}>
                  <li className={`Shopping-bag__form-list__item`}>
                    <Flex justify={"space-between"}>
                      <Title bodyText={"p"}>Order value</Title>
                      <Title bodyText={"p"}>{cartTotal}$</Title>
                    </Flex>
                  </li>

                  <li className={`Shopping-bag__form-list__item`}>
                    <Flex justify={"space-between"}>
                      <Title bodyText={"p"}>Shipping:</Title>
                      <Title bodyText={"p"}>0.00$</Title>
                    </Flex>
                  </li>

                  <li className={`Shopping-bag__form-list__item`}>
                    <Flex justify={"space-between"}>
                      <Title bodyText={"p"}>Total</Title>
                      <Title bodyText={"p"}>82.00$</Title>
                    </Flex>
                  </li>
                </ul>
                ...
                <Btn
                  type={"submit"}
                  primary
                  className={`Shopping-bag__form-button--primary`}
                >
